...
class DumpFileExtractor(DumpExtractor):
    ...
    def extract_nodes(self):
        ...
        if not os.path.exists(filename_path):
            with open(filename_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                nodewriter.writerow(["article_id", "article_title", "article_namespace"])

                for article in self:
                    if article.redirect_title is None:
                        nodewriter.writerow([article.id, article.title.strip(), article.ns])

    def extract_redirections(self):

        # Redirections
        filename_path = os.path.join(self._directory_name, self.dump.redirection_filename)
        if not os.path.exists(filename_path):
            with open(filename_path, 'w', newline='', encoding='utf-8') as csvfile:
                redirectionwriter = csv.writer(csvfile, delimiter=',',
                                               quotechar='|', quoting=csv.QUOTE_MINIMAL)
                redirectionwriter.writerow(["article_id", "article_title", "redirection_title"])

                for article in self:
                    if article.redirect_title is not None:
                        redirectionwriter.writerow([article.id, article.title.strip(), article.redirect_title.strip()])
                    ...
                    if article.redirect_title is None:
                        if len(article.title.strip()) <= maxlength_article_title:
                            for link in article.links:
                                linkwriter.writerow([article.id, article.title.strip(), link.strip()])
    ...
    def extract_categories(self):
        ...
        if not os.path.exists(filename_path):
            with open(filename_path, 'w', newline='', encoding='utf-8') as csvfile:
                categorywriter = csv.writer(csvfile, delimiter=',',
                                               quotechar='|', quoting=csv.QUOTE_MINIMAL)
                ...
                for article in self:
                    if article.redirect_title is None:
                        for category in article.categories:
                            categorywriter.writerow([article.id, article.title.strip(), category.strip()])
    ...
    def extract_portals(self):
        ...
        if not os.path.exists(filename_path):
            with open(filename_path, 'w', newline='', encoding='utf-8') as csvfile:
                portalwriter = csv.writer(csvfile, delimiter=',',
                                               quotechar='|', quoting=csv.QUOTE_MINIMAL)
                ...
                for article in self:
                    if article.redirect_title is None:
                        for portal in article.portals:
                            portalwriter.writerow([article.id, article.title.strip(), portal.strip()])
